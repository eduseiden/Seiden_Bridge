...
def evo_driver_command(
    reader: dict[str, Any],
    command: str,
    request_timeout: int,
    **kwargs: Any,
) -> dict[str, Any]:
    """Executa um comando no driver EVO."""
    payload = {
...
def execute_reader_command(
    ...
    **kwargs: Any,
    ...
    """Encaminha um comando ao driver configurado para o leitor."""
    driver = reader.get("driver", "evo")

    if driver == "evo":
        return evo_driver_command(
            reader=reader,
            command=command,
    ...
    raise RuntimeError(
        f"Driver '{driver}' ainda não implementado nesta versão"
    )
    ...
    for reader in readers:
        ...
        if reader.get("enabled", True) and driver not in SUPPORTED_DRIVERS:
            raise RuntimeError(
                ...
                "ainda não está implementado na versão 0.6.0. "
